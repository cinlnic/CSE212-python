...
class Translator:
    ...
    def add_word(self, from_word, to_word):
        ...
        """
        # add the english word and the key and the german word as the value
    ...
    def translate(self, from_word):
        ...
        """
        # check to see if the key is in the dictionary, if it is return the translation otherwise return ???
...
def summarize_degrees(filename):
    ...
    with open(filename) as file_in:
        for line in file_in:
            ...
            # check if the degree is in the dictionary, if not add it as the key and a value of 1
            if fields[3] not in degrees:
                degrees[fields[3]] = 1
            # if the degree is in the dictionary, increase the value by 1
            else:
                degrees[fields[3]] += 1
...
print(summarize_degrees("census.txt"))
...
def is_anagram(word1, word2):
    ...
    # change words to lower case and remove spaces
    w1 = word1.lower().replace(" ", "")
    w2 = word2.lower().replace(" ", "")

    # create summary tables for each word of how many times each letter is in the word
    for letter in w1:
   ...
   # compare the dictionaries to see if the key value pairs are equal
    return (dict1 == dict2)
...
class Maze:
    ...
    def move_left(self):
        ...
        """
        # get the values of current position
        move = self.maze_map[(self.curr_x, self.curr_y)]

       # if the value at the current position is true update the position otherwise print error msg
        if move[0] == True:
            self.curr_x -= 1
        else:
...
def earthquake_daily_summary():
    ...
    # The .json() function will convert the json data from the server to a dictionary
    data = req.json()

    # ADD YOUR CODE HERE
    # loop through each dictionary in features
    for features in data["features"]:
        # retrieve the location and magnitude for each feature(earthquake) and print it
        loc = features["properties"]["place"]
        mag = features["properties"]["mag"]

        print("{} - Mag {}".format(loc, mag))
