...
class BST:
    ...
    def _get_height(self, node):
        ...
        if node.left is not None and node.right is not None:
            leftHeight = self._get_height(node.left) + 1
            rightHeight = self._get_height(node.right) + 1
...
def _insert_middle(sorted_list, first, last, bst):
    ...
    #if there is only one element in the sorted listjust insert it into the tree
    if len(sorted_list) == 1:
        BST.insert(bst, sorted_list[0])

    #find the middle of the sorted list, then insert it into the tree
    middle = (last + first) // 2
    BST.insert(bst, sorted_list[middle])
    #the last then becomes the index of the middle to add the right side
    last = sorted_list.index(sorted_list[middle])

    if last is not 0:
        _insert_middle(sorted_list, 0, last, bst)
        
    #then the first becomes the middle index to add the left side of the sorted array        
    first = sorted_list.index(sorted_list[middle])
    if first is not 0:
